...
@section('main-title', 'Nueva Factura')
...
@section('main')
    ...
    @php( $iva = \App\Setting::getValueByName('iva')->toArray()[0]['setting_value'] )
    <form action="{{route('invoices.store')}}" method="POST">
        ...
        <div class="card">
                        ...
                        <div class="row">
                            <div class="col-md text-md-left">
                                ...
                                <div id="client_data">
                                    <p>
                                        @if(old('client'))
                                            <?php $client = \App\Client::find(old('client')) ?>
                                            {!! $client['dni'].',<br>'.$client['name'].' '.$client['last_name'] !!}
                                        @endif
                                    </p>
                                </div>
                                <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#searchClient" id="searchClientButton">Seleccionar Cliente</button>
                            ...
                            <div class="col-md text-md-center">
                                ...
                                <div id="vehicle_data">
                                    @if(old('vehicle'))
                                        <?php $vehicle = \App\Vehicle::find(old('vehicle')) ?>
                                        <p>{!! $vehicle['vehicle_registration'].',<br>'.$vehicle->model->brand->name.' '.$vehicle->model['name'] !!}</p>
                                    @endif
                                </div>
                                <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#searchVehicle" id="searchVehicleButton">Seleccionar Vehículo</button>
                                <div class="text-danger">
                            ...
                            <div class="col-md text-md-right">
                                ...
                                <input type="number" style="border:0px;" name="vehicle_kilometers" min="0" class="form-control text-right {{(strlen($errors->first('vehicle_kilometers'))>0) ? 'is-invalid' : ''}} {{(strlen($errors->first('vehicle_kilometers'))==0 && sizeof($errors)>0) ? 'is-valid' : ''}}" value="{{(old('vehicle_kilometers')) ? old('vehicle_kilometers') : 0}}">
            ...
            <div class="card-footer bg-white">
                <button class="btn btn-primary" type="submit">Añadir factura</button>
            </div>
        </div>
        @include('admin.create.modals.invoices')
        <input type="hidden" id="client" name="client" value="{{old('client')}}">
        <input type="hidden" id="vehicle" name="vehicle" value="{{old('vehicle')}}">
